...
import { DEFAULT_ARENA_ID, getArena, getCheckpoint, requireArena } from './arenas';
...
export function createInitialPortfolio(arenaId: ArenaId = DEFAULT_ARENA_ID): PortfolioState {
  return requireArena(arenaId).startingPortfolio();
}
...
export function createInitialRun(
  ...
  const total = requireArena(arenaId).checkpoints.length;
