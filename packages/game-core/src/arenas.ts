...
import type { ArenaId, CheckpointData, PortfolioState } from './types';
...
/**
 * An arena the registry does not have.
 *
 * Registration is composition, and composition happens outside this package:
 * the authored arena modules register themselves into this registry and the
 * application wires them up before it opens a run. That makes a missing
 * registration a configuration error, not a content gap, and the two want
 * opposite handling.
 *
 * The engine used to answer a missing registration with a hard-coded COVID
 * book. In the browser that state is unreachable, so the fallback only ever
 * hid a mistake. In the server-side verifier the growth architecture calls for
 * (REFI_ALPHA_GROWTH_ARCHITECTURE.md §6) it would be worse than hiding one: a
 * forgotten registration would verify a ranked attempt against the wrong book
 * and report a confident, wrong score. A thrown error is the only answer that
 * cannot be mistaken for a result.
 *
 * A missing CHECKPOINT keeps its old meaning — the end of a run — because an
 * arena that is registered and has run out of content is an ordinary state.
 */
export class UnregisteredArenaError extends Error {
  readonly arenaId: string;
  constructor(arenaId: string) {
    super(
      `arena "${arenaId}" is not registered. Arenas register themselves when their `
      + 'module is imported; the composition root has to import them before a run is '
      + 'opened.',
    );
    this.name = 'UnregisteredArenaError';
    this.arenaId = arenaId;
  }
}

/** An arena, or an explicit failure naming the one that is missing. */
export function requireArena(id: ArenaId): ArenaDefinition {
  const arena = REGISTRY.get(id);
  if (!arena) throw new UnregisteredArenaError(id);
  return arena;
}
