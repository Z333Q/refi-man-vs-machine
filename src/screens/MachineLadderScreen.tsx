...
export default function MachineLadderScreen({ onChallenge, onBack }: Props) {
  ...
  // The ladder row is the decision (which machine); the action zone is the
  // commit. Default to the first machine actually available to challenge.
        ...
        <div className="space-y-3">
          {[...MACHINE_LADDER].reverse().map((machine, revIdx) => {
            ...
            return (
              <div
                key={machine.id}
                className={`border p-4 sm:p-5 transition-all duration-200 ${STATUS_COLORS[status]} ${
                  isTaco && isActive ? 'border-alert-amber/50 bg-alert-amber/5' : ''
                } ${isSelected ? 'ring-1 ring-phosphor/40' : ''}`}
